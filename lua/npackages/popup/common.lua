...
local M = {
	TOP_OFFSET = 2,
	POPUP_NS = vim.api.nvim_create_namespace("npackages.nvim.popup"),
	LOADING_NS = vim.api.nvim_create_namespace("npackages.nvim.popup.loading"),
}
...
function M.win_height(entries)
	return math.min(#entries + M.TOP_OFFSET, state.cfg.popup.max_height)
end
...
local function set_buf_content(buf, title, text)
	...
	-- vim.api.nvim_buf_set_option(buf, "filetype", "npackages.nvim")
	vim.bo[buf].modifiable = false
	vim.bo[buf].buftype = "nofile"
	vim.bo[buf].swapfile = false
	vim.bo[buf].filetype = "npackages.nvim"
	vim.api.nvim_buf_set_name(buf, "npackages:popup")
end
